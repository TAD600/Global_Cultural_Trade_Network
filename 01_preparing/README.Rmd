...
`01_preparing_trade_data.ipynb`: Loads trade data from the OECD BIMTS bulk data files at 2 digit HS code level. Cleans and prepares data on non-cultural merchandise trade, unique cultural goods trade and reproducible cultural trade (books, cinema and tapes).
...
`03_preparing_edgelists_for_SNA.ipynb`: Prepares edgelists for unique and reproducible cultural products, non-cultural product trade and respective hysteresis effects.
...
`05_unit_root_test.do`: Stata do file that runs IPS tests and adds first differences values of non-stationary variables to `attributes2000_2023.csv` dataset.

`06_preparing_panel_data_&_removing_countries.ipynb` : Loads all eleven edgelists and the 'node attribute' data prepared in the previous notebook, finds out the countries those are not present in the attribute data and removes those from all edgelists and replaces the earlier instances of the edgelist csv files. 
